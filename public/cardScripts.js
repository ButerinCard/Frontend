...
document.querySelectorAll(".cardForContract_svg__fidenzaColor").forEach((element) => {
    element.setAttribute("fill", fidenzaPalette[colorId]);
});
if (colorId >= 14) {
    document.querySelectorAll(".cardForContract_svg__patternLines").forEach((element) => {
        element.setAttribute("fill", "white");
    });
}
document.querySelectorAll(".cardForContract_svg__rayColor").forEach((element) => {
    element.setAttribute("stop-color", fidenzaPalette[colorId]);
});

// Settings the pattern
document.getElementById("cardForContract_svg__frame").setAttribute("fill", `url(#pattern${patternId})`);
document.getElementById("cardForContract_svg__frameBlurry").setAttribute("fill", `url(#pattern${patternId})`);
...
document.querySelectorAll(".cardForContract_svg__rotate").forEach((element) => element.setAttribute("patternTransform", bgRotate));

// Formatting title box
const ethTitleElement = document.getElementById("cardForContract_svg__eth_title")
switch (phaseId) {
    case 0:
        ethTitleElement.setAttribute("href", "#cardForContract_svg__eth_pencil");
        ethTitleElement.setAttribute("transform", "translate(85.5 9.1) scale(.015 .015)");
        break;
    case 1:
        ethTitleElement.setAttribute("href", "#cardForContract_svg__eth_bw");
        ethTitleElement.setAttribute("transform", "translate(87 8.7) scale(.01 .01)");
        break;
    case 2:
        ethTitleElement.setAttribute("href", "#cardForContract_svg__eth_grey");
        ethTitleElement.setAttribute("transform", "translate(87 8.7) scale(.01 .01)");
        break;
    case 3:
        ethTitleElement.setAttribute("href", "#cardForContract_svg__eth_color_a");
        ethTitleElement.setAttribute("transform", "translate(87 8.7) scale(.01 .01)");
        break;
    case 4:
        ethTitleElement.setAttribute("href", "#cardForContract_svg__eth_color_b");
        ethTitleElement.setAttribute("transform", "translate(87 8.7) scale(.01 .01)");
        break;
    default:
}
...
let d = `M10 8 l-2 ${rnd() < 0.5 ? "0" : "2"} V12.82842712475 l${rnd() < 0.5 ? "0" : "2"} 2 H90 l2 ${rnd() < 0.5 ? "0" : "-2"
    } V10 l${rnd() < 0.5 ? "0" : "-2"} -2 Z`;
document.getElementById("cardForContract_svg__titlePath").setAttribute("d", d);
document.getElementById("cardForContract_svg__titlePathOutside").setAttribute("d", d + dRevert);

d = `M10 83.1084271247 l-3 ${rnd() < 0.5 ? "0" : "3"} V86.1084271247 l${rnd() < 0.5 ? "0" : "3"} 3 H90 l3 ${rnd() < 0.5 ? "0" : "-3"
    } V86.1084271247 l${rnd() < 0.5 ? "0" : "-3"} -3 z`;
document.getElementById("cardForContract_svg__subtitlePath").setAttribute("d", d);
document.getElementById("cardForContract_svg__subtitlePathOutside").setAttribute("d", d + dRevert);

// Setting the protrusions for the image element
const generateProtrudedPath = function (y0, height) {
    const transition = function (pos) {
...
document.getElementById("cardForContract_svg__imagePath").setAttribute("d", d);
document.getElementById("cardForContract_svg__imagePathOutside").setAttribute("d", d + dRevert);
d = generateProtrudedPath(91.1084271247, 40.8915728753);
document.getElementById("cardForContract_svg__descriptionPath").setAttribute("d", d);
document.getElementById("cardForContract_svg__descriptionPathOutside").setAttribute("d", d + dRevert);
...
lines.forEach((line, ind) =>
    document
        .getElementById(`cardForContract_svg__line${Math.floor((7 - lines.length) / 2) + ind + 1}`)
        .appendChild(document.createTextNode(line))
);
document.getElementById("cardForContract_svg__lineKB").appendChild(document.createTextNode(`Size = ${kiloBytes} KB`));
...
document.fonts.ready.then(() => {
    ...
    document.getElementById("cardForContract_svg__numberWrap").setAttribute("width", tokenIdInPhaseElement.getBBox().width + 1.1);

    const descritionTextWidth = lines
        .map((undefined, ind) => document.getElementById(`line${Math.floor((7 - lines.length) / 2) + ind + 1}`).getBBox())
        .reduce((maxWidth, { width }) => Math.max(maxWidth, width), 0);
    document.getElementById("cardForContract_svg__descriptionText").setAttribute("transform", `translate(${(100 - descritionTextWidth) / 2 - 14})`);
});

// Setting the subtitle
const subtitleElement = document.getElementById("cardForContract_svg__subtitleText");
subtitleElement.appendChild(document.createTextNode(subtitleText));

// Setting the card ID
document.getElementById("cardForContract_svg__cardNumber").appendChild(document.createTextNode(`${tokenId + 1}`));

// Set special card attributes
if (phaseId) document.getElementById("cardForContract_svg__vitalik").setAttribute("href", "#vitalikImage");
if (subtitleText === "Bitcoiner") {
    if (phaseId) document.getElementById("cardForContract_svg__laserEyesImage").setAttribute("visibility", "visible");
    else document.getElementById("cardForContract_svg__laserEyesPencil").setAttribute("visibility", "visible");
}
if (subtitleText === "WoW")
    document.getElementById("cardForContract_svg__speechBubble").setAttribute("visibility", "visible");

if (subtitleText === "The Hard Forkoooooor")
    document.getElementById("cardForContract_svg__pitchfork").setAttribute("visibility", "visible");